...
contract NumberGame is ReentrancyGuard {
    ...
    function createGame() public payable returns (uint256) {
        ...
        require(
            games[gameId].currentState == GameState.GameEnded ||
                games[gameId].currentState == GameState.NewGame,
            "create game - game state error"
        );
        require(msg.value >= defaultMinBet,"Please send more than default minimum bet to join the game");
        address payable currentPlayer = payable(msg.sender);
        games[gameId] = Game({
            player1: currentPlayer,
            player2: payable(address(0)),
            player1Bet: msg.value, // Default value
    ...
    function joinGame(uint16 gameId) public payable {
        ...
        require(
            game.currentState == GameState.NewGame ||
    ...
    function makeGuess(uint16 gameId, uint8 guessNumber) public payable {
        ...
        require(
            (isPlayer1 && !game.p1BetStatus) ||
                (!isPlayer1 && !game.p2BetStatus),
    ...
    function finalizeGame(uint16 gameId) internal {
        ...
        if (
            game.player1Guess == currentTargetNumber &&
            game.player2Guess == currentTargetNumber
        ) {
    ...
    function withdraw(uint16 gameId) public nonReentrant {
        ...
        require(
            game.player1Bet > 0 || game.player2Bet > 0,
