...
describe("NumberGame tests set #1", function () {
    ...
    describe("createGame", function () {
        it("should create a new game with the correct initial state", async function () {
            const createGameTx = await numberGame.connect(addr1).createGame({value: ethers.utils.parseEther("0.05")});
    ...
    describe("joinGame", function () {
        ...
        it("should not allow owner to join the game", async function () {
            const tx = await numberGame.connect(addr1).createGame({value: ethers.utils.parseEther("0.05")});
        ...
        it("should not allow player2 to join with mismatched bet", async function () {
            const tx = await numberGame.connect(addr1).createGame({value: ethers.utils.parseEther("0.06")});
            const receipt = await tx.wait();
            const gameId = receipt.events[0].args.gameId;
            await expect(
    ...
    describe("makeGuess and finalizeGame", function () {
        it("should allow players to make guesses, place bets, and automatically finalize the game", async function () {
            // Create a new game
            const gameTx = await numberGame.connect(addr1).createGame({value: ethers.utils.parseEther("0.05")});
...
describe("NumberGame tests set #2", function () {
    ...
    describe("NumberGame administrative functions", function () {
        ...
        describe("getTargetNumber", function () {
            it("should allow owner to get the target number", async function () {
                tx = await numberGame.connect(addr1).createGame({value: ethers.utils.parseEther("0.05")});
                receipt = await tx.wait();
        ...
        describe("setNextGameId", function () {
            ...
            it("should successfully create a game and increment nextGameId", async function () {
                await numberGame.connect(addr1).createGame({value: ethers.utils.parseEther("0.05")});
            ...
            it("should create a game and set its properties correctly", async function () {
                ...
                expect(gameAfterCreation.currentState).to.equal(GameState.WaitingForPlayer);
                expect(gameAfterCreation.player1).to.equal(addr1.address);
    ...
    describe("ownerWithdraw function", function () {
        it("should allow the owner to withdraw", async function () {
            tx = await numberGame.connect(addr1).createGame({value: ethers.utils.parseEther("1")});
            const receipt = await tx.wait();
            const gameId = receipt.events[0].args.gameId;
        ...
        it("should not allow non-owners to withdraw", async function () {
            const tx = await numberGame.connect(addr1).createGame({value: ethers.utils.parseEther("1")});
            const receipt = await tx.wait();
            const gameId = receipt.events[0].args.gameId;
            const game = await numberGame.games(gameId);
            await numberGame.connect(addr1).withdraw(gameId);
